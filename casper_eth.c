...
casper_netif_output_impl(struct netif *netif, struct pbuf *p)
{
  ...
  xil_printf("CASPER_ETH: send %u bytes as %u longwords from addr %p [ms %u]\n",
  ...
  if(i == 1000) {
    ...
    xil_printf("CASPER_ETH: error: previous tx still in progress\n");
    LINK_STATS_INC(link.drop);
    return ERR_INPROGRESS;
  }
#ifdef VERBOSE_ETH_IMPL
  else xil_printf("CASPER_ETH: looped %d times waiting for previous tx to finish\n", i);
#endif // VERBOSE_ETH_IMPL
  ...
  while(p) {
    ...
    len = p->len;
    if(p->len > p->tot_len) {
      xil_printf("CASPER_ETH: YIKES p->len > p->tot_len (%u > %u), using p->tot_len\n",
          p->len, p->tot_len);
      len = p->tot_len;
    }
...
#ifdef VERBOSE_ETH_IMPL
    xil_printf("CASPER_ETH: copying %u of %u bytes from pbuf %p payload %p to tx buf %p\n",
...
#ifdef DEBUG_PKT_TX
  outbuf32 = TX_BUF_PTR32(ifstate.ptr);
  xil_printf("CASPER_ETH: TX device buf %p:\n", outbuf32);
  for(i=0; i<8; i++) {
  ...
  *TX_BUF_SIZE_PTR16(ifstate.ptr) = (words_sent >> 1);
  //*TX_BUF_SIZE_PTR16(ifstate.ptr) = (words_sent << 2);

#ifdef VERBOSE_ETH_IMPL
  xil_printf("CASPER_ETH: sent pkt %u bytes as %u longwords [ms %u]\n",
  ...
  if(i == 1000) {
    ...
    xil_printf("CASPER_ETH: warning: packet tx is slow\n");
  }
...
casper_netif_output_loop(struct netif *netif, struct pbuf *p)
{
  ...
  for(i=0; i<3; i++) {
    ...
    if(rc != ERR_OK) {
      xil_printf("CASPER_ETH: casper_netif_output_impl returned %d\n", rc);
      return rc;
    }
  }
  return ERR_OK;
...
casper_netif_status_callback(struct netif *netif)
{
...
#ifdef VERBOSE_ETH_IMPL
  xil_printf("CASPER_ETH: casper_netif_status_callback()\n");
#endif // VERBOSE_ETH_IMPL
  ...
  if(netif->ip_addr.addr != 0) {
    if((rc = casper_tftp_init()) != ERR_OK) {
      xil_printf("CASPER_ETH: TAPCP server startup failed (%d)\n", rc);
    } else {
      xil_printf("CASPER_ETH: TAPCP server ready\n");
    }
  }
#ifdef VERBOSE_ETH_IMPL
  else print("CASPER_ETH: no ip address, NOT starting TAPCP\n");
#endif // VERBOSE_ETH_IMPL
}
...
casper_lwip_init()
{
  ...
  if(!ifstate.ptr) {
    print("CASPER_ETH: No ethernet cores found!\n");
    return ERR_IF;
  }

  print("CASPER_ETH: using ethernet core ");
  print((const char *)core_name);
  print("\n");

#ifdef DEBUG_ETH0_MEM
  print("CASPER_ETH: ## eth0 memory as uint32_t:\n");
  for(i=0; i<4; i++) {
    xil_printf("%02x:", 16*i);
    for(j=0; j<4; j++) {
...
casper_rx_packet()
{
  ...
  struct pbuf *p, *q;
  //u32 *eth0_ptr32 = (u32 *)ifstate.ptr;

  // Check for received frames, feed them to lwIP
  if(!(size64 = *RX_BUF_SIZE_PTR16(ifstate.ptr))) {
      /*k = k + 1;
      if (k%128 == 0){
      print("CASPER_ETH: ## No packet received. eth0 memory as u32:\n");
      for(i=0; i<4; i++) {
        xil_printf("%02x:", 16*i);
        for(j=0; j<4; j++) {
      ...
      print("CASPER_ETH: ## RX buffer\n");
      for(i=0; i<4; i++) {
        xil_printf("%04x:", (16*i+0x2000));
        for(j=0; j<4; j++) {
          xil_printf(" %08x", eth0_ptr32[4*i+0x2000+j]);
        }
        print("\n");
      }
      print("\n");
  ...
  // Work with words
  //xil_printf("PACKET SIZE64: %d\n", size64);
  //size32 = size64 << 1;
  size32 = (size64 >> 2) + 1;

  //xil_printf("PACKET SIZE32: %d\n", size32);
  // If packet too large
  if(size32 > sizeof(rx_swapb)) {
    ...
    *RX_BUF_SIZE_PTR16(ifstate.ptr) = 0;
    ...
    LINK_STATS_INC(link.drop);
    xil_printf("CASPER_ETH: pkt too big %u > %u words [ms %u]\n",
  ...
  if(!p) {
    ...
    *RX_BUF_SIZE_PTR16(ifstate.ptr) = 0;
    LINK_STATS_INC(link.memerr);
    LINK_STATS_INC(link.drop);
    xil_printf("CASPER_ETH: bad pbuf for %d bytes [ms %u]\n",
        (size32<<2), ms_tmrctr());
    // Nothing more to do here
    return;
  }
  ...
  //size32 = size64 >> 2;
#ifdef VERBOSE_ETH_IMPL
  xil_printf("CASPER_ETH: read %u bytes as %u longwords to addr %p [ms %u]\n",
...
#ifdef VERBOSE_ETH_IMPL
  xil_printf("CASPER_ETH: byte swapping %u words to rx_swapb\n", size32);
#endif // VERBOSE_ETH_IMPL
  for(i=0; i<size32; i++) {
...
#ifdef DEBUG_PKT_RX
  nibuf32 = SWAPB_BUF_PTR32(ifstate.ptr);
  xil_printf("CASPER_ETH: RX byte-swapped buf %p:\n", nibuf32);
  for(i=0; i<8; i++) {
    if((i & 3) == 0) xil_printf("%04x:", (i<<2));
  ...
  while(q) {
    ...
    if(bytes_remaining <= q->len) {
      ...
      if(q != p) {
        ...
        print("CASPER_ETH: YIKES pkt rx truncating non-first pbuf!\n");
      }
    }
    // Copy bytes
  ...
  while (1) 
{
  ...
  if(i > 0) {
    xil_printf("CASPER_ETH: looped %d times waiting for TX buffer\n\n", i);
  }

  print("CASPER_ETH: send pkt32 ver: b\n");

  //random_addr = Xil_In32(XPAR_AXI_SLAVE_WISHBONE_CLASSIC_MASTER_0_BASEADDR + ETH_MAC_REG8_RX_ENABLE);
  //xil_printf("Random addr: %d\n", random_addr);
  for(i=0; i<16; i++) {
    xil_printf("Addr %d : %d\n", i, ((uint32_t *)ifstate.ptr)[i]);
  }

  xil_printf("Macro IP: %d\n",*MACRO_IP_PTR32(ifstate.ptr)); 
  ...
  *TX_BUF_SIZE_PTR16(ifstate.ptr) = pktlen32*4;
  //*TX_BUF_SIZE_PTR16(ifstate.ptr) = j;

  xil_printf("Length sent: %d\n", *TX_BUF_SIZE_COPY(ifstate.ptr));
  ...
  //xil_printf("pkt32: Set size: %d\n",pktlen32*4);


  print("CASPER_ETH: send pkt8\n");
  ...
  // Set TX buffer size to number of 8 byte words to send packet
  *TX_BUF_SIZE_PTR16(ifstate.ptr) = pktlen8;
  xil_printf("pkt8: Set size: %d\n",pktlen8);
  j=j+1;
}
}
